...
import { CheckCircle, Trash2, Lightbulb } from 'lucide-react';
...
export default function SuggestionManager() {
  ...
  const markReviewed = async (item: Suggestion) => {
    const { error } = await supabase.from('suggestions').update({ is_reviewed: !item.is_reviewed }).eq('id', item.id);
    if (error) { showToast(t('Failed to update', 'فشل التحديث'), 'error'); return; }
    fetchData();
  };

  const handleDelete = async (item: Suggestion) => {
    if (!confirm(t('Delete this suggestion?', 'حذف هذا الاقتراح؟'))) return;
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="font-display text-2xl sm:text-3xl font-bold text-green-800">{t('Suggestions', 'الاقتراحات')}</h1>
          <p className="text-sm text-green-600/70 mt-1">{items.length} {t('entries', 'إدخالات')}</p>
        </div>
        <div className="flex gap-2">
          {(['all', 'new', 'reviewed'] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={`px-3 py-2 rounded-lg text-sm font-semibold transition-all ${filter === f ? 'bg-caramel-500 text-cream-50' : 'bg-white text-caramel-700 hover:bg-caramel-50 border border-caramel-100'}`}>
      ...
      {items.length === 0 && (
        <div className="text-center py-16">
          <Lightbulb className="h-12 w-12 text-green-300 mx-auto mb-3" />
          <p className="text-green-600 font-semibold">{t('No suggestions yet', 'لا توجد اقتراحات')}</p>
        </div>
      )}
    </div>
  );
